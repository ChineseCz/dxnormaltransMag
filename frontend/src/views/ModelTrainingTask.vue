    ...
    <div class="flex justify-between items-center">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 class="text-lg font-bold text-white tracking-tight leading-tight">模型训练任务调度</h1>
          <p class="text-slate-400 text-xs mt-0.5">深度学习 / 机器学习模型训练过程管理与实时监控</p>
        </div>
      </div>
        ...
        <div class="space-y-4">
          ...
          <div class="p-3 rounded-xl space-y-3" style="background:rgba(2,8,23,0.6); border:1px solid rgba(51,65,85,0.4);">
            ...
            <div class="flex items-center gap-1.5">
              <span class="w-1.5 h-1.5 rounded-full flex-shrink-0" :style="`background:${currentModel.color}`"></span>
              <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">运行环境</span>
            </div>
            <!-- 模型标识卡片 -->
            ...
            <div class="grid grid-cols-2 gap-x-3 gap-y-2">
              <div>
                ...
                <div class="text-[11px] text-slate-300 font-mono">4 → 1241</div>
              </div>
              <div>
...
import { VideoPlay, CircleClose, Monitor, DataAnalysis, Refresh, Connection, Grid, Timer, Aim, Share } from '@element-plus/icons-vue';
...
const dnnLayerStr = computed(() => {
  ...
  if (!layers?.length) return '4→…→1241';
  return `4→${layers.map(l => l.units).join('→')}→1241`;
});
