      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-slate-500 text-xs mt-1.5 ml-[48px]">
            铁心电抗器 COMSOL 2D 轴对称仿真 · 激励电流 100 A → 磁通密度三维场分布
          </p>
        </div>
    ...
    <div class="px-6 pb-3">
      <div class="rounded-xl px-5 py-4 space-y-3"
        ...
        <div class="flex items-center gap-3 flex-wrap pt-1"
          ...
          <span class="text-slate-400 text-[10px] font-mono">DNN_2025-09-12_09-10-22.pth</span>
          <div class="flex items-center gap-1.5 ml-2">
            <span class="text-[10px] text-slate-500">精度：</span>
            <span class="metric-chip">MAE <b class="text-cyan-400">1.19×10⁻⁴</b> T</span>
            <span class="metric-chip">MAPE <b class="text-cyan-400">0.45%</b></span>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;padding:14px;">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <div style="height:430px;" class="rounded-lg overflow-hidden">
              <WebGL3DScatter :points="gtPoints" unit="T" label="B"
                :box-width="100" :box-height="60" :box-depth="100" class="w-full h-full"
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <div style="height:430px;" class="rounded-lg overflow-hidden">
              <WebGL3DScatter :points="dnnPoints" unit="T" label="B"
                :box-width="100" :box-height="60" :box-depth="100" class="w-full h-full"
                @render-time="onChartRendered" />
            </div>
          </div>
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <div style="height:430px;" class="rounded-lg overflow-hidden">
              <WebGL3DScatter :points="diffPoints" unit="T" label="|ΔB|"
                :box-width="100" :box-height="60" :box-depth="100" class="w-full h-full"
                @render-time="onChartRendered" />
            </div>
          </div>
...
const displayParams = [
  { label: '激励电流 I：',  value: '280',    unit: 'A'  },
  { label: '总节点数 N：',  value: '91,462', unit: '个' },
  { label: '场量',        value: 'B',      unit: 'T'  },
];
...
let _fetchEnd    = 0;
let _renderCount = 0;
const e2eMs      = ref(null);

function onChartRendered(ms) {
  _renderCount++;
  if (_renderCount === 3) {
    const total = Math.round(performance.now() - _fetchStart);
    e2eMs.value = total;
    console.log(
      `[ReactorField E2E] API: ${Math.round(_fetchEnd - _fetchStart)} ms  ` +
      `| JS数据处理: ${Math.round(_fetchEnd - _fetchStart)} ms → WebGL(3×30K): ~${ms} ms  ` +
      `| 端到端合计: ${total} ms  | 总渲染点数: 90,000`
    );
  }
}
/**
 * 模拟 DNN 预测值：叠加约 ±0.5% 峰值幅度的随机噪声
 * 对应论文 MAE = 1.19×10⁻⁴ T，R² = 0.99
 */
...
async function loadField() {
  _fetchStart  = performance.now();
  _renderCount = 0;
  e2eMs.value  = null;

  let gt = [];
  try {
    const resp = await fetch('http://127.0.0.1:5000/api/reactor/field3d?n_base=2500&n_angles=12&current=280');
    const data = await resp.json();
    if (data.points?.length) gt = data.points;
  } catch (e) { console.warn('[ReactorField] 后端请求失败，使用演示数据', e); }
  if (!gt.length) gt = genDemo(20000);

  _fetchEnd = performance.now();
  console.log(
    `[ReactorField] API响应: ${Math.round(_fetchEnd - _fetchStart)} ms | ` +
    `${gt.length} 点 | JSON约 ${Math.round(JSON.stringify(gt).length/1024)} KB`
  );
...
function genDemo(n = 20000) {
  ...
  for (let i = 0; i < base; i++) {
    const r = Math.random() * 3500;
    const z = (Math.random() - 0.5) * 4800;
    const inCore = r > 700 && r < 2100 && Math.abs(z) < 1200;
    const B = inCore
      ? 0.002 + 0.006 * Math.exp(-((r - 1400) * (r - 1400)) / 300000 - z * z / 800000) * (0.9 + 0.1 * Math.random())
      : 0.0001 * Math.random();
    for (let j = 0; j < N_A; j++) {
      const theta = (2 * Math.PI * j) / N_A;
      pts.push({ x: r * Math.cos(theta), y: r * Math.sin(theta), z, value: Math.max(0, B) });
    }
  }
  return pts;
}
