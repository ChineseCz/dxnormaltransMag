  ...
  <div class="min-h-full" style="background:#060d1a;">
    ...
    <div class="px-6 pb-6 space-y-5">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <span class="text-slate-400 text-sm font-semibold">
            共 <span class="text-amber-400 font-bold">{{ filteredHistory.length }}</span> 条记录
            <span v-if="filteredHistory.length !== history.length" class="text-slate-600 text-xs ml-1">
              （共 {{ history.length }} 条）
            </span>
          </span>
        ...
        <div class="flex items-center gap-2">
          <el-select v-model="pageSize" size="small" style="width:90px;"
            ...
            <el-option v-for="n in [5, 10, 20]" :key="n" :label="`${n} 条/页`" :value="n" />
          </el-select>
      ...
      <TransitionGroup name="list" tag="div" class="space-y-3">
        <div v-for="(record, idx) in pagedHistory" :key="record.id"
          ...
          <div class="flex items-start gap-4">
            <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold"
                 :style="`background:${indexColors[globalIdx(idx) % indexColors.length]}15; color:${indexColors[globalIdx(idx) % indexColors.length]};`">
              {{ globalIdx(idx) + 1 }}
            </div>
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-3 text-[11px] text-slate-500">
                ...
                <span class="flex items-center gap-1"><el-icon size="11"><Folder /></el-icon>{{ record.datasetName || record.datasetId || '—' }}</span>
      ...
      <!-- 分页 -->
      <div v-if="filteredHistory.length > pageSize"
...
function handleDelete(record) {
  ElMessageBox.confirm(
    ...
    // 删完当页最后一条时退回上一页
    if (pagedHistory.value.length === 1 && currentPage.value > 1) {
      currentPage.value--;
    }
...
:deep(.el-pagination .el-input__inner) { color: #e2e8f0 !important; }
</style>
