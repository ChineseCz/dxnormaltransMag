      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-slate-500 text-xs mt-1.5 ml-[48px]">
            高压套管 FEM 仿真 · 60 Hz · 激励电流 1000 A → 功率损耗密度三维场分布
          </p>
        </div>
    ...
    <div class="px-6 pb-3">
      <div class="rounded-xl px-5 py-4 space-y-3"
        ...
        <div class="flex items-center gap-3 flex-wrap pt-1"
          ...
          <span class="text-slate-400 text-[10px] font-mono">DNN_2025-07-25-19-28-48.pth</span>
          <div class="flex items-center gap-1.5 ml-2">
            <span class="text-[10px] text-slate-500">精度：</span>
            <span class="metric-chip">MAE <b class="text-cyan-400">1.12×10⁻²</b> W/m³</span>
            <span class="metric-chip">MAPE <b class="text-cyan-400">2.05%</b></span>
            <span class="metric-chip">R² <b class="text-green-400">0.995</b></span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="px-6 pb-6">
      <div class="chart-card">
        <div class="chart-header">
          ...
          <span class="text-[11px] text-slate-500">功率损耗密度 Po_r (W/m³)</span>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;padding:14px;">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <div style="height:430px;" class="rounded-lg overflow-hidden">
              <WebGL3DScatter :points="gtPoints" unit="W/m³" label="Po_r"
                class="w-full h-full" />
            </div>
          </div>
          <!-- (b) DNN 预测 -->
          <div class="flex flex-col gap-1.5">
            ...
            <div style="height:430px;" class="rounded-lg overflow-hidden">
              <WebGL3DScatter :points="dnnPoints" unit="W/m³" label="Po_r"
                class="w-full h-full" />
            </div>
          </div>
          <!-- (c) 差值 -->
          <div class="flex flex-col gap-1.5">
            ...
            <div style="height:430px;" class="rounded-lg overflow-hidden">
              <WebGL3DScatter :points="diffPoints" unit="W/m³" label="|ΔPo_r|"
                class="w-full h-full" />
            </div>
          </div>
        </div>
...
const displayParams = [
  { label: '激励电流 I',  value: '600',     unit: 'A'    },
  { label: '激励频率 f',  value: '60',      unit: 'Hz'   },
  { label: '总节点数 N',  value: '238,222', unit: '个'   },
  { label: '场量',        value: 'Po_r',    unit: 'W/m³' },
];
...
async function loadField() {
  let gt = [];
  try {
    const resp = await fetch('http://127.0.0.1:5000/api/gaoya/field3d?current=600');
    const data = await resp.json();
    if (data.points?.length) gt = data.points;
  } catch (e) { console.warn('[GaoyaField] 后端请求失败，使用演示数据', e); }
  if (!gt.length) gt = genDemo(5000);
...
function genDemo(n = 5000) {
  const pts = [];
  for (let i = 0; i < n; i++) {
    const r   = Math.random() * 25;
    const ang = Math.random() * 2 * Math.PI;
    const x   = r * Math.cos(ang);
    const z3  = r * Math.sin(ang);
    const y   = Math.random() * 10350;
    const po  = r < 15 ? Math.abs(800 * Math.exp(-r / 8) * (0.8 + 0.2 * Math.random())) : 0;
    pts.push({ x, y, z: z3, value: po });
  }
  return pts;
}
