...
class NetShortCircuit(nn.Module):
    def __init__(self):
        ...
        self.cov2 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=2, stride=1)
...
# x = torch.randn(BATCH_SIZE, 1, 4).requires_grad_(True).cuda()  # 定义一个网络的输入值
# y = net(x)
# pic_1 = make_dot(y, params=dict(list(net.named_parameters()) + [('x', x)]))
# pic_1.format = "jpg"  #定义pic为模型可视化后的输出,这里输出为png格式
#  # 指定文件生成的文件夹pic里面
# pic_1.directory = "./modelSummary"
#  # 生成文件
# pic_1.view()
