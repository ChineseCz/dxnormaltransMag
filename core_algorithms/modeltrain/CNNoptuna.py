...
def define_model(trial):
    ...
    layers = []
    # 先添加一个卷积层
    layers.append(nn.Conv1d(in_channels=1, out_channels=64, kernel_size=2, stride=1))
...
def objective(trial):
    ...
    np.savetxt(projectPath + '/optuna_result/CNN/trail{}.txt'.format(trial.number), predY, header=header)
...
if __name__ == "__main__":
    ...
    os.makedirs(projectPath + '/optuna_result/CNN/BestTrial{}'.format(trial.number))
    picSavePath = projectPath + '/optuna_result/CNN/BestTrial{}'.format(trial.number)
