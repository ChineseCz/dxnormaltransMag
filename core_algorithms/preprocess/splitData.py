...
def splitFiles():
    ...
    for i in range(outputs.shape[0]):
        ...
        data2 = inputs[i, :]
        print(data2.shape)
        np.savetxt('../data/splited data/output/{:.4f}.txt'.format(t0 + 5e-4 * i), data1)
        np.savetxt('../data/splited data/input/{:.4f}.txt'.format(t0 + 5e-4 * i), data2)
...
def combineTrainTest():
    ...
    y_train = np.zeros((1, 1241))
    X_test = np.zeros((1, 4))
    y_test = np.zeros((1, 1241))
    ...
    print(y_train.shape)   # (96, 1241)
    print(X_test.shape)    # (25, 4)
    print(y_test.shape)    # (25, 1241)
...
def indataNormalize():
    ...
    :return:
    '''
    trainInputdata = np.loadtxt('../data/trainInput.txt')   # (96, 4)
    testInputdata = np.loadtxt('../data/testInput.txt')  # (24, 4)

    # 对训练集做归一化
    new_trainindata = np.zeros((96, 1))
    train_mu = np.zeros((1, 1))
    train_sigma = np.zeros((1, 1))
    for i in range(trainInputdata.shape[1]):
    ...
    new_trainindata = np.delete(new_trainindata, 0, axis=1)  # (64, 15)
    ...
    # 测试集的归一化有问题
    # 测试集数据归一化（利用训练集数据的mu和sigma
    new_testdata = np.zeros((25, 1))
    for j in range(testInputdata.shape[1]):
...
def rebuild_PCA():
    ...
    print('rmse: ', np.sqrt(mean_squared_error(raw_data, huanyuan_data)))
    print('mae: ', mean_absolute_error(raw_data, huanyuan_data))
